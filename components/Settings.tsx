...
import { 
  ...
  Database, Cloud, CloudOff, RefreshCw, Server, ExternalLink, Code
} from 'lucide-react';
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {activeTab === 'cloud' && (
            <section className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100 animate-in fade-in duration-300 space-y-8">
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 gap-4">
                   <div>
                    ...
                    <input type="text" placeholder="https://xyz.supabase.co" value={settings.cloud?.supabaseUrl} onChange={e => setSettings({...settings, cloud: {...settings.cloud!, supabaseUrl: e.target.value}})} className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500" />
                   </div>
                   <div>
                    <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Service Role API Key (Anon Key)</label>
                    <input type="password" placeholder="eyJhbGci..." value={settings.cloud?.supabaseKey} onChange={e => setSettings({...settings, cloud: {...settings.cloud!, supabaseKey: e.target.value}})} className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500" />
                   </div>
                </div>

                <div className="flex gap-3">
                   <button onClick={testCloud} disabled={testing} className="flex-1 py-3 bg-white border border-slate-200 text-slate-700 font-bold rounded-xl text-sm hover:bg-slate-50 transition-all flex items-center justify-center gap-2">
              ...
              <div className="bg-slate-900 rounded-2xl p-6 text-white space-y-4">
                 ...
                 <p className="text-xs text-slate-400">Copy and run this SQL in your Supabase dashboard to provision the necessary tables for MedAuth AI:</p>
                 <pre className="bg-black/50 p-4 rounded-xl text-[10px] font-mono text-emerald-400 overflow-x-auto whitespace-pre">
                   {sqlSchema}
                 </pre>
              </div>
            </section>
        ...
        <div className="space-y-6">
          <div className="bg-gradient-to-br from-slate-800 to-slate-900 p-8 rounded-3xl shadow-xl text-white border border-slate-700">
            ...
            <p className="text-slate-400 text-sm mb-6 leading-relaxed">An active BAA is on file. All data is processed using Gemini 3.0 Pro Enterprise-grade security.</p>
            <div className="p-3 bg-white/10 rounded-xl border border-white/10 text-xs font-mono">CERT_ID: 9823-XJ-BAA</div>
          </div>
          ...
          <div className="p-8 bg-rose-50 rounded-3xl border border-rose-100">
            ...
            <button onClick={() => { if(window.confirm('WIPE ALL DATA? This will clear all local drafts and history.')) { localStorage.clear(); window.location.reload(); } }} className="w-full py-2.5 bg-rose-600 text-white font-bold rounded-xl hover:bg-rose-700 transition-all text-sm flex items-center justify-center gap-2"><Trash2 size={16} /> Reset Practice Data</button>
          </div>
        </div>
      </div>
    </div>
