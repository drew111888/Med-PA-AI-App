    ...
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20 animate-pulse">
           ...
           <p className="text-slate-500 font-medium">Fetching clinical guidelines...</p>
        </div>
      ) : (
            ...
            <div key={policy.id} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 hover:border-blue-300 transition-all group flex flex-col">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  ...
                  <button onClick={async () => { if(window.confirm('Delete?')) { await deletePolicy(policy.id); loadPolicies(); } }} className="text-slate-400 hover:text-red-600"><Trash2 size={16} /></button>
                </div>
              </div>
